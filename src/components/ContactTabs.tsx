...
type TabId = "message" | "calendar" | "whatsapp";
    ...
    <>
      ...
      <div className="flex border-t border-b border-rule/100 md:ml-[72px] reveal">
        ...
        <div
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") setActive("calendar");
          }}
      ...
      <div className="flex min-h-[580px] md:ml-[72px]">
        <div className="flex-1 px-6 py-10 md:p-[60px]">
          ...
          {active === "calendar" && (
            <CalendarPanel calendarEmbedUrl={calendarEmbedUrl} />
          )}
  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-[280px_1fr] gap-8 md:gap-[60px] items-start">
      <div className="w-full">
        ...
        <h2 className="text-[28px] font-bold leading-[1.25] mb-4 tracking-tight-1">
          Pick a time that works for you.
        </h2>
        <p className="text-[15px] text-mid leading-[1.75] mb-7">
          Choose a 30 or 60 minute slot from Prem&apos;s live calendar. You&apos;ll
          receive a confirmation and Google Meet link immediately.
        </p>

        <CalDetail>
          <svg viewBox="0 0 24 24" className="w-4 h-4 flex-shrink-0 fill-none stroke-red stroke-[1.8]" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="w-full bg-warm border border-rule/100 rounded-[2px] overflow-hidden min-h-[520px] flex items-center justify-center relative">
        {calendarEmbedUrl ? (
          <iframe
            ...
            title="Book a call with Prem Menon"
            style={{ width: "100%", height: "520px", border: 0 }}
          />
        ) : (
          <div className="flex flex-col items-center justify-center gap-4 p-10 text-center">
            <div className="w-14 h-14 border-[1.5px] border-rule/100 rounded-lg flex items-center justify-center">
              <svg
                className="w-7 h-7 fill-none stroke-red stroke-[1.5]"
                viewBox="0 0 24 24"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
            ...
            <p className="text-[13px] text-light leading-[1.6] max-w-[260px]">
              Set <code className="text-[11px] bg-white border border-rule/100 px-[10px] py-1 rounded-[2px] text-mid tracking-wide-1">NEXT_PUBLIC_CALENDAR_EMBED_URL</code> to enable the live scheduler.
            </p>
          </div>
        )}
      </div>
...
function CalDetail({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 text-sm text-mid mb-[14px]">
      {children}
    </div>
  );
}
